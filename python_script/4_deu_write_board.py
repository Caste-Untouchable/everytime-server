...
for i in get_board_pk():
    for ii in range(10):
        index = random.randint(0, comment_max-1)
        payload['boardTypePK'] = i
        payload['boardTitle'] = arr[index]
        index = random.randint(0, comment_max-1)
...
# 2. 학교 보드 PK 값을 받아온다.(리스트로 야무지게 파싱해서 저장)

# 3. 학교 보드 PK 값, 게시글 제목, 게시글 내용, 유저 ID (testID_1) 을 딕셔너리에 넣는다.

# 4. 적절히 반복문을 활용하여 게시글1 게시글2 이런식으로 반복한다

# 5. 딕셔너리를 json으로 변환하여 Body에 넣는다.
